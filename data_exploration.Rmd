...
# move columns around to match layout of first CDOM spreadsheet
water_spectral_indices_part2 <- water_spectral_indices_part2[,c(13,1,14,2:12)]
...
water_corr_plot <- water_CDOM_final[,c(9,10,12,14:19)]
...
```{r}
# rerun the loop with all Great Lakes together to better look at correlations on plots

# make copy of dataset so edits don't override existing dataframe
water_soil_carbon_edit <- water_soil_carbon_noNA

# add a new level to the water_body variable
water_soil_carbon_edit$water_body <- factor(water_soil_carbon_edit$water_body, levels = c(levels(water_soil_carbon$water_body), "Great Lakes"))

# edit water_body variable to merge the Great Lakes together
for (n in 1:130) {
  # check if row is in the Great Lakes region and transfer that to the water_body column if it's true
  if (water_soil_carbon_edit$region[n] == "Great Lakes") {
    water_soil_carbon_edit$water_body[n] <- "Great Lakes"
  }
}
...
for (n in 1:3) {
  # will cycle through the loop one time for each site location
  filtered_data <- water_soil_carbon_edit %>%
    filter(transect_location == location_list[n])
