...
# merge water characteristics with CDOM information
water_info <- water_soil %>%
  select(campaign, kit_id, state, region, contains("water")) %>%
  unique() %>%
  filter(!is.na(state))

water_cdom_info <- water_cdom %>%
  filter(kit_id %in% extracts_cdom$kit_id) %>%
  left_join(water_info, by = "kit_id") %>%
  rename(ph = water_ph) %>%
...
extracts_cdom_info <- extracts_cdom %>%
  filter(!is.na(SUVA254)) %>%
  left_join(select(water_soil, campaign:water_systemtype, transect_location:percent_sand)) %>%
  relocate(campaign:water_systemtype, .before = SUVA254)

# bind extracts and water together
cdom_water_extracts <- bind_rows(extracts_cdom_info, water_cdom_info)
...
```{r}
# set up data frame
pca_extracts_cdom <- cdom_water_extracts[,c(8,9,11,13:18)]
...
cdom_groups <- as.data.frame(cdom_water_extracts[,2])
...
ggplot(filter(cb_cdom_water_extracts, transect_location == "Water"), aes(x = water_sal_psu, y = SUVA254, color = transect_location)) +
  geom_point()
