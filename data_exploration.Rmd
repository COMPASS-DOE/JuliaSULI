...
# merge these filtered kits back with soil mass info
filter_kits_masses <- left_join(filter_available_kits, soil_availability_reduce)

# merge soil metadata for TC % back with the giant data frame
filter_kits_masses <- left_join(filter_kits_masses, water_soil_carbon)

# add column to calculate current amount of soil for each kit/transect location
filter_kits_masses$available_mass_after_xrd <- filter_kits_masses$post_sieving_weight_from_tare_g - 1

# clean up the data frame
filter_kits_masses <- filter_kits_masses[,-c(16:19,21:33)]

# add a column for the kits that have already been done!!
filter_kits_masses <- filter_kits_masses %>%
  mutate(analyzed = ifelse(kit_id == "K019" |
           kit_id == "K013" |
           kit_id == "K010" |
           kit_id == "K050" |
           kit_id == "K061" |
...
# separate out by water body
GL_filtered_kits <- filter_kits_masses %>%
  filter(water_body == "Great Lakes")
CB_filtered_kits <- filter_kits_masses %>%
  filter(water_body == "Chesapeake Bay")
DB_filtered_kits <- filter_kits_masses %>%
  filter(water_body == "Delaware Bay")

# info for just kits already analyzed to use as a reference
kits_analyzed <- filter_kits_masses %>%
  filter(kit_id == "K019" |
           kit_id == "K013" |
           kit_id == "K010" |
           kit_id == "K050" |
  ...
  relocate(transect_location, .after = kit_id) %>%
  relocate(tc_perc, .after = transect_location)
```

```{r}
# DELAWARE BAY SITES
# just a note - all kits here have sediments as well
# remove sediment information since values are NA anyway
DB_filtered_kits <- DB_filtered_kits %>%
  filter(transect_location != "Sediment")

# add new column for potential for analysis
DB_filtered_kits <- DB_filtered_kits %>%
  mutate(good_soil_amount = ifelse(available_mass_after_xrd > 21,
                                   1,
                                   0))

# summarize DB kits
DB_kits_summary <- DB_filtered_kits %>%
  group_by(kit_id, analyzed, water_longitude, water_latitude) %>%
  summarize(status = sum(good_soil_amount))

DB_kits_summary$status <- as.factor(DB_kits_summary$status)

# map the DB information
ggplot(data = usa) +
  geom_sf() +
  coord_sf(xlim = c(-75.75,-74.75), ylim = c(38.9,40)) +
  geom_point(data = DB_kits_summary, mapping = aes(x = water_longitude, y = water_latitude, color = status, shape = analyzed), size = 2.5) +
  theme_bw() +
  labs(x = "Longitude", y = "Latitude", color = "Transect Locations with Enough Soil", shape = "Analyzed Already?")
  
```

```{r}
# CHESAPEAKE BAY SITES
# Kit 51 is the only one without sediment collected
# remove sediment rows
CB_filtered_kits <- CB_filtered_kits %>%
  filter(transect_location != "Sediment")
```
...
final_extract_sites <- target_sites_filtered %>%
  filter(kit_id == "K019" |
           kit_id == "K013" |
           kit_id == "K010" |
           kit_id == "K050" |
...
original_extract_sites <- target_sites_filtered %>%
    filter(kit_id == "K019" |
           kit_id == "K013" |
           kit_id == "K010" |
           kit_id == "K039" |
